...
const GroupedRow = ({ card, count, onRemove }: { card: DeckCard, count: number, onRemove: () => void }) => (
    <div className="flex items-center gap-3 bg-navy-900 border border-navy-800 p-2 rounded mb-1 relative group hover:border-cyan-500/50 transition-colors">
        ...
        {/* Remove Button (removes one copy) */}
        <button 
           onClick={onRemove}
...
const Section = ({ title, cards, icon, onRemove }: { title: string, cards: DeckCard[], icon: any, onRemove: (id: string) => void }) => {
    // Group logic
            ...
            <div>
                {rows.map(row => (
                    <GroupedRow 
                        ...
                        card={row.card} 
                        count={row.count} 
                        onRemove={() => onRemove(row.instanceIds[0])} // Remove one instance
                    />
                ))}
            </div>
        </div>
...
export const ListView = () => {
    ...
    return (
        <div className="p-4 overflow-y-auto h-full pb-32">
             <Section title="MAIN DECK" cards={mainDeck} icon={<Layers className="w-4 h-4"/>} onRemove={(id) => removeCard(id, 'main')} />
             <Section title="EXTRA DECK" cards={extraDeck} icon={<Box className="w-4 h-4"/>} onRemove={(id) => removeCard(id, 'extra')} />
             <Section title="SIDE DECK" cards={sideDeck} icon={<Archive className="w-4 h-4"/>} onRemove={(id) => removeCard(id, 'side')} />
        </div>
    );
};
